...
- [**Beekeeper Studio**](https://www.beekeeperstudio.io/) - open source, cross platform, and the most pleasant of these to actually sit in front of. Covers SQL Server, PostgreSQL, SQLite and Redis, so like DBeaver it is one tool for most of the [Data](07-postgresql.html) part of this book. The free community edition is enough for querying and editing day to day; the paid edition adds things like the query magics and backup/restore UI.
- [**DBeaver**](https://dbeaver.io/) - open source and cross platform, and the one to pick when breadth matters most. It reaches databases nothing else here does, and the community edition is fully featured, at the cost of a heavier, more cluttered interface than Beekeeper's.
