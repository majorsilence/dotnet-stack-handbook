#!/usr/bin/env bash
#
# Build the handbook's PDF from the Markdown chapters.
#
# LaTeX `book` class, 10pt, two-sided, parts and numbered chapters with an
# appendix, a table of contents, and coloured hyperlinks.
#
# Requires: pandoc, and a LaTeX engine (xelatex by default, pdflatex works too).
#
#   ./tools/build-pdf.sh                    # -> build/dotnet-stack-handbook.pdf
...
output="build/dotnet-stack-handbook.pdf"
while [ $# -gt 0 ]; do
  case "$1" in
...
# level-1 heading, and rewrite the Jekyll-only conventions that pandoc does not
# read: cross-chapter links become plain intra-document anchors once every chapter
# lives in one file.
...
for path in sorted(glob.glob("_chapters/*.md")):
    ...
    # ("](03-structuring-an-application.html)" -> "](#03-structuring-an-application)")
    # land on it.
    attrs = ["#" + stem]
    if meta.get("unnumbered") == "true":
